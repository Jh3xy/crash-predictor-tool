...
export class UIController {
    constructor(elements) {
        ...
        this.predictorCard = document.getElementById('predictor-card');
    ...
    renderPrediction(result) {
        ...
        const confidenceValue = confidenceVal.toFixed(1); 
        const confidencePercent = confidenceValue + '%';

        // Set the CSS variable for the confidence bar width on the card element
        if (this.predictorCard) {
            // Cap confidence at 100% and ensure it's a string with % sign
            const clampedConfidence = Math.min(confidenceVal, 100).toFixed(1) + '%';
            this.predictorCard.style.setProperty('--confidence-level', clampedConfidence);
        }
